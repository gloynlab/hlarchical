...
print(os.getcwd())
modeldict_file = f'models/models.yaml'
with open(modeldict_file) as f:
    modeldict = yaml.safe_load(f)

def _get_num_classes(dataset: str, hla: str, digit: str, data_dir) -> int:
    '''
    Args:
...
def implement_model(dataset_name, hla_list, digit, input_len, chunk_len, settings, data_dir):
    models = {}
    models['shared'] = _implement_model_shared(settings, input_len, chunk_len)
    for hla in hla_list:
        class_num = _get_num_classes(dataset_name, hla, digit, data_dir)
...
def _implement_model_shared(settings: Dict, input_len, chunk_len) -> nn.Module:
    ...
    foot_module = import_module(f'models.architectures.foots.{module_name_f}')
    ...
    pe_module = import_module(f'models.architectures.pos_encode.{module_name_p}')
    ...
    shared_module = import_module(f'models.architectures.shared_net.{module_name_s}')
...
def _implement_model_each(settings: Dict, class_num: int) -> nn.Module:
    ...
    each_module = import_module(f'models.architectures.each_net.{module_name_e}')
